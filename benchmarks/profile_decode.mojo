...
def main() raises:
    ...
    for _ in range(20):
        var total: Int = 0
        for id in ids:
            total += tok.token_table.arena.spans[id].length
    ...
    for _ in range(20):
        var total2: Int = 0
        for id2 in ids:
            total2 += tok.token_table.arena.spans[id2].length
        var out2 = List[UInt8](capacity=total2)
        for id2 in ids:
            var start2 = tok.token_table.arena.spans[id2].offset
            var end2 = start2 + tok.token_table.arena.spans[id2].length
            for r2 in range(start2, end2):
                out2.append(tok.token_table.arena.bytes[r2])
    ...
    for _ in range(20):
        var total3: Int = 0
        for id3 in ids:
            total3 += tok.token_table.arena.spans[id3].length
        var out3 = List[UInt8](capacity=total3)
        for id3 in ids:
            var start3 = tok.token_table.arena.spans[id3].offset
            var end3 = start3 + tok.token_table.arena.spans[id3].length
            var r3 = start3
            while r3 < end3:
                if r3 + 1 < end3 and tok.token_table.arena.bytes[r3] == 0xC4 and tok.token_table.arena.bytes[r3 + 1] == 0xA0:
                    out3.append(UInt8(0x20))
                    r3 += 2
                else:
                    out3.append(tok.token_table.arena.bytes[r3])
                    r3 += 1
    ...
    var total4: Int = 0
    for id4 in ids:
        total4 += tok.token_table.arena.spans[id4].length
    var prebuilt = List[UInt8](capacity=total4)
    for id4 in ids:
        var start4 = tok.token_table.arena.spans[id4].offset
        var end4 = start4 + tok.token_table.arena.spans[id4].length
        for r4 in range(start4, end4):
            prebuilt.append(tok.token_table.arena.bytes[r4])
