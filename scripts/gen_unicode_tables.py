...
def emit(bounds, masks, meta, unicode_note):
    ...
    func_defs.append(
        ...
        '    """Like \\\\p{Lu}\\\\p{Lt}\\\\p{Lm}\\\\p{Lo}\\\\p{M} for o200k (non-Ll letters + marks).\n\n'
        "    Letters that are NOT lowercase (\\\\p{Ll}) plus combining marks.  The\n"
        "    \\\\p{L} set is exact, so digits can never be classified as letters here\n"
        "    (no digit prune needed).\n"
        '    """\n'
        "    if cp < 128:\n"
        "        return 65 <= cp <= 90\n"
    ...
    func_defs.append(
        ...
        '    """Like \\\\p{Ll}\\\\p{Lm}\\\\p{Lo}\\\\p{M} for o200k (lowercase + other letters + marks).\n\n'
        "    The o200k letter classes overlap on Lm/Lo/M; only \\\\p{Lu}\\\\p{Lt} are\n"
        "    excluded here so the case-split between alt 1 and alt 2 is exact.\n"
        '    """\n'
        "    if cp < 128:\n"
        "        return 97 <= cp <= 122\n"
        "    var m = _class_mask(UInt32(cp))\n"
...
# Unicode {unicode_note}
# {N} event-point boundaries, {len(distinct)} distinct class masks, ~{(N * 5) / 1024:.1f} KiB.
#
# Event-point table: entry i covers codepoints [BOUNDS[i], BOUNDS[i+1]); the
# last entry covers [BOUNDS[N-1], 0x10FFFF].  _class_mask() binary-searches
# the largest boundary <= cp and returns the 6-bit class mask for that
# interval.  See UNICODE_TABLES.md for the design.

from std.collections.inline_array import InlineArray

# 6-bit class mask layout (see UNICODE_TABLES.md).
comptime BIT_L: UInt8 = {BIT_L}  # \\p{{L}}   (letters)
comptime BIT_N: UInt8 = {BIT_N}  # \\p{{N}}   (numbers)
comptime BIT_l: UInt8 = {BIT_l}  # \\p{{Ll}}  (lowercase letters)
comptime BIT_u: UInt8 = {BIT_u}  # \\p{{Lu}}|\\p{{Lt}} (uppercase + titlecase letters)
comptime BIT_M: UInt8 = {BIT_M}  # \\p{{M}}   (combining marks)
comptime BIT_W: UInt8 = {BIT_W}  # White_Space
...
def _class_mask(cp: UInt32) -> UInt8:
    \"\"\"Return the 6-bit class mask for a codepoint (binary search).\"\"\"
    var lo: Int = 0
    var hi: Int = {N - 1}
    while lo < hi:
        var mid = (lo + hi + 1) >> 1
